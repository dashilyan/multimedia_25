...
class ArtifactDetect {
    ...
    private fun detectBlur(gray: Mat, size: Size): List<DetectedArtifact> {
        val artifacts = mutableListOf<DetectedArtifact>()
        val blockSize = 80 // Увеличен размер блока
        val minBlockStdDev = 5.0 // Порог плоскости блока

        // Вычисление глобального порога по всей картинке
        ...
        val dynamicBlurThreshold = fullStdDev.toArray()[0] * 0.7 // Подгоняемо
        ...
        for (y in 0 until gray.height() step blockSize) {
            for (x in 0 until gray.width() step blockSize) {
                ...
                // Фильтрация слишком "плоских" блоков
                ...
                // Laplacian и стандартное отклонение
                val laplacian = Mat()
                ...
                // Детекция размытия по относительному уровню
